rule univariate_test:
    input:
        feature_path=lambda wildcards: canonical_module_table_path(wildcards.dataset_id),
        sample_metadata_path=metadata_input,
    output:
        result=f"{RESULTS_DIR}/univariate_test/{{dataset_id}}/{{method}}/result_matrices.rds",
    log:
        f"{LOGS_DIR}/univariate_test/{{dataset_id}}.{{method}}.log",
    params:
        contrast=join_space(config["univariate"]["contrast"]),
        group_key_levels=join_space(config["univariate"]["group_order"]),
